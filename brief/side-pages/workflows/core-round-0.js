...
You are the integration engineer for ROUND 0. Implement ${ROOT}/brief/side-pages/core/CORE-SPEC.md completely — sections A, B, C, D, E and F — in this order:
1. Read the spec, then HANDOFF §3–§4, CONTRACT.md, DEPLOY.md, theme/snippets/elmsnest-v2-base.liquid, theme/sections/elmsnest-v2-hero.liquid (top 10 lines), theme/layout/theme.liquid, brief/inventory/theme-src/snippets/css-variables.liquid (l.60–140 and l.210–250), brief/inventory/INVENTORY.md §2, brief/side-pages/OWNER-NOTES.md.
2. Take the baseline: copy brief/inventory/home/http-desktop.png, http-mobile.png, http-desktop-fold.png, http-mobile-fold.png to brief/side-pages/core/before-home-<name>.png.
3. Write the new/changed files under ${ROOT}/theme/ mirroring theme paths: snippets/elmsnest-v2-core.liquid, snippets/elmsnest-v2-ground-index.liquid, snippets/elmsnest-v2-photo-url.liquid, snippets/elmsnest-v2-base.liquid (stub), sections/elmsnest-v2-hero.liquid (line 7 + photo-url), sections/elmsnest-v2-switch.liquid, -terms, -goodnight (photo-url), layout/theme.liquid (minimal edit — but FETCH THE LIVE FILE from the theme first and diff it against the repo copy; edit the live content), templates/<each edited>.json (fetch live, edit minimally, save the edited copy under theme/templates/ and theme/templates/customers/), sections/system-group.json (fetch, edit, save), snippets/css-variables.liquid (fetch, edit ONLY the --en-* values, save), config/settings_data.json (fetch, surgical edits, save — never rewrite from memory; backslashes pass literally in block strings).
4. Run python3 brief/lint.py after extending it per §F.1; fix until LINT OK.
5. Deploy in the §F.2 order, one file per themeFilesUpsert call; stop and fix on any userErrors; log filename → size.
6. Mirror + shoot the §F.3 pages (JS-enabled http shots). Then run the §F.4 acceptance checks with real measurements: PIL pixel samples (12 points per page: the header band behind the menu at y≈35 CSS px at x = 20%,40%,60%,80%; four points in the main content; four near the footer seam), grep counts, heights; the cart-drawer interaction with Playwright over python3 -m http.server (stub /cart/add.js with page.route; screenshot the drawer at 1440 and 390 to brief/side-pages/core/drawer-desktop.png / drawer-mobile.png). Read your PNGs personally (Read tool) before declaring anything legible.
7. Write ${ROOT}/brief/side-pages/core/REPORT.md per §F.5.
Rules: keep every change minimal and reversible; keep the homepage pixel-identical (compare the before/after folds with PIL: mean absolute difference per channel — report the number); no WhatsApp label anywhere; do not delete elms-pdp-* sections; do not touch templates/index.json. Return the structured result (paths relative to the repo).`, { label: 'engineer:core', phase: 'Build', schema: BUILD_SCHEMA, effort: 'high' })
