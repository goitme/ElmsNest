...
async function audit(page, name, vw, vh) {
  return page.evaluate(({ name, vh, FEAT }) => {
    ...
    { const T = (main.innerText || ''); const FAM = { path: ['בלילה, על השביל', 'אור נמוך, קרוב לאדמה, שמלווה את הדרך.'], wall: ['בלילה, על הקיר', 'אור על הקיר, בדיוק בנקודה שבחרתם.'], spot: ['בלילה, בגינה', 'אור מכוון אל מה שרוצים לראות בלילה.'], decor: ['בלילה, במרפסת', 'הרבה נקודות אור קטנות במקום מנורה אחת. אור שיושבים בו.'], home: ['בערב, בבית', 'הרבה נקודות אור קטנות במקום מנורה אחת. אור שיושבים בו.'] }; const fam = Object.keys(FAM).find(k => T.includes(FAM[k][0])) || null; const need = fam ? [...FAM[fam], 'למדריך לבחירת תאורה'] : []; if (q('.ens-pdp-dusk').length) need.push('כשמחשיך', 'ביום הפאנל אוסף שמש, וזה מה שמאיר בלילה.', 'יותר שמש ביום, יותר אור בלילה.'); res.pdpCopy = { family: fam, missing: need.filter(t => !T.includes(t)), dusk: q('.ens-pdp-dusk').length, credit: T.includes('בתמונה:') }; }
