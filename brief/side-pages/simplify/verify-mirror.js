...
async function audit(page, name, vw, vh) {
  return page.evaluate(({ name, vh, FEAT }) => {
    ...
    // pdp round 2 (pdp2/SPEC.md §5): the scene section (photo by the map, text block, one guide link), the dusk row (solar gate)
    res.pdpSections = q('.ens-pdp-scene, .ens-pdp-dusk, .ens-pdp-weather, .ens-pdp-print').map(sec => { const imgs = [...sec.querySelectorAll('img')]; return { section: sec.className.split(' ').find(c => c.startsWith('ens-pdp-')), imgs: imgs.length, sized: imgs.filter(i => i.getAttribute('width') && i.getAttribute('height')).length, lazy: imgs.filter(i => i.loading === 'lazy').length, loaded: imgs.filter(i => i.complete && i.naturalWidth > 0).length, srcset: imgs.map(i => (i.getAttribute('srcset') || '').split(',').map(s => s.trim().split(' ').pop()).join('/')).join(' '), credits: sec.querySelectorAll('a[href*="/products/"]').length, h: Math.round(sec.getBoundingClientRect().height), text: (sec.innerText || '').replace(/\s+/g, ' ').trim().slice(0, 200) }; });
    res.guideLinksInMain = q('main a[href*="guide-garden-lighting"], #MainContent a[href*="guide-garden-lighting"]').length;
    { const T = (main.innerText || ''); const FAM = { path: ['בלילה, על השביל', 'אור נמוך, קרוב לאדמה, שמלווה את הדרך.'], wall: ['בלילה, על הקיר', 'אור על הקיר, בדיוק בנקודה שבחרתם.'], spot: ['בלילה, בגינה', 'אור מכוון אל מה שרוצים לראות.'], portable: ['בלילה, בשטח', 'אור מכוון אל מה שרוצים לראות.'], decor: ['בלילה, במרפסת', 'הרבה נקודות אור קטנות במקום מנורה אחת. אור שיושבים בו.'], home: ['בערב, בבית', 'הרבה נקודות אור קטנות במקום מנורה אחת. אור שיושבים בו.'] }; const fam = Object.keys(FAM).find(k => T.includes(FAM[k][0])) || null; const need = fam ? [...FAM[fam], 'למדריך לבחירת תאורה'] : []; if (q('.ens-pdp-dusk').length) need.push('כשמחשיך', 'ביום הפאנל אוסף שמש, וזה מה שמאיר בלילה.', 'יותר שמש ביום, יותר אור בלילה.'); if (q('.ens-pdp-weather').length) need.push('בחוץ, כל השנה', 'גשם של ינואר.', 'שמש של אוגוסט.', 'בוגנוויליה על קיר, בשמש של צהריים'); if (q('.ens-pdp-print').length) need.push('האור האחרון של היום.'); const cred = t => (t || '').replace(/\s+/g, ' '); res.pdpCopy = { family: fam, missing: need.filter(t => !T.includes(t)), dusk: q('.ens-pdp-dusk').length, credit: T.includes('בתמונה:'), weather: q('.ens-pdp-weather').length, print: q('.ens-pdp-print').length, credits: q('.ens-pdp-weather .ens-pdp-weather__credit, .ens-pdp-print .ens-pdp-print__credit').map(e => cred(e.innerText).trim()).filter(Boolean), printCap: cred((q('.ens-pdp-print .ens-pdp-print__cap')[0] || {}).innerText || '').trim() }; }
