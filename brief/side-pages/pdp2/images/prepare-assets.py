#!/usr/bin/env python3
"""Turn chosen photographs into DEV-THEME assets — PDP round 2 (assets/ens-pdp-<slug>.jpg). Frames and manifest come from ../../home2/images/.
...
quality stepping down from 82 until ≤ kb), write theme/assets/ens-pdp-<slug>.jpg, a BASE64 themeFilesUpsert
mutation to brief/side-pages/pdp2/deploy/NN-assets__ens-pdp-<slug>.jpg.graphql, and append the choice with its
...
SRC = os.path.join(os.path.dirname(H), 'home2', 'images')  # the frames and the manifest of the home round (fetch.py own)
man = {json.loads(l)['id']: json.loads(l) for l in open(os.path.join(SRC, 'manifest.jsonl'), encoding='utf-8')}
n = 0
for spec in args:
    ...
    rec = man[pid]
    im = ImageOps.exif_transpose(Image.open(os.path.join(SRC, rec['file']))).convert('RGB')
    if crop:
