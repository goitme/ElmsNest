// Usage: node brief/shot-drawer.js <path/to/index.html> <out-prefix> [--rm]
// Like shot-http.js, but opens the Kalles cart drawer before the screenshot (viewport shot, not fullPage).
...
(async () => {
  ...
  for (const [name, vp] of [['desktop', { width: 1440, height: 900 }], ['mobile', { width: 390, height: 844 }]]) {
    ...
    const opened = await p.evaluate(async () => {
      const host = document.querySelector('hdt-cart-drawer');
      const dlg = document.getElementById('CartDrawer');
      if (!dlg) return 'no-dialog';
      let how = 'showModal';
      try {
        if (host && typeof host.open === 'function') { host.open(); how = 'element.open()'; }
        else if (typeof dlg.show === 'function' && dlg.getAttribute('pos')) { dlg.showModal(); }
        else dlg.showModal();
      } catch (e) { try { dlg.showModal(); } catch (e2) { return 'failed:' + e2.message; } }
      dlg.setAttribute('open', '');
      await new Promise(r => setTimeout(r, 400));
      return how + (dlg.open ? ' open' : ' NOT-open');
    });
    console.log(`${name}: drawer=${opened}`);
    await p.waitForTimeout(2800);
    await p.screenshot({ path: `${outPrefix}-${name}.png`, fullPage: false });
